...
bool SingleProjectiveIntegrator::updateVoxel(
  ...
  if (apply_normal_refine && input.has(InputData::InputType::kNormalImage)
    ...
    if (voxel->gradient.norm() > kFloatEpsilon) { // use current not yet updated normal because the weight is unknown
      v_C = T_C_S.getRotationMatrix() * voxel->gradient; // transform back to sensor(camera)'s frame
